...
class CalendarByPeriodsView(CalendarMixin, DetailView):
    ...
    def dispatch(self, request, *args, **kwargs):
        print("Chequeando permisos")
        if not User.objects.filter(Q(pk=request.user.id),Q(groups__name='Doctor') | Q(groups__name='Secretaria')).exists():
            print("No hay permisos!!")
            return render(request,'agenda/permisos.html',{})
        return super(CalendarByPeriodsView, self).dispatch(request, *args, **kwargs)

    @classmethod
    def as_view(cls, **initkwargs):
        view = super(CalendarByPeriodsView, cls).as_view(**initkwargs)
...
class EventView(EventMixin, DetailView):
    ...
    @classmethod
    def as_view(cls, **initkwargs):
        view = super(EventView, cls).as_view(**initkwargs)
        return login_required(view)
...
class EditEventView(EventEditMixin, UpdateView):
    ...
    @classmethod
    def as_view(cls, **initkwargs):
        view = super(EditEventView, cls).as_view(**initkwargs)
        return login_required(view)
...
class CreateEventView(EventEditMixin, CreateView):
    ...
    @classmethod
    def as_view(cls, **initkwargs):
        view = super(CreateEventView, cls).as_view(**initkwargs)
        return login_required(view)
    form_class = EventForm
    template_name = 'agenda/create_event.html'#Este template fue modificado para que renderice los templates de medc
...
class DeleteEventView(EventEditMixin, DeleteView):
    ...
    @classmethod
    def as_view(cls, **initkwargs):
        view = super(DeleteEventView, cls).as_view(**initkwargs)
        return login_required(view)
    def get_context_data(self, **kwargs):
